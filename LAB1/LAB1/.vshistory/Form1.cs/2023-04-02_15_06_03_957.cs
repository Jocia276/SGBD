...
namespace LAB1
{
    public partial class Form1 : Form
    {
        ...
        DataSet cds, childds = new DataSet();
        ...
        private void update_prod_Click(object sender, EventArgs e)
        {
            try
            {
                using (SqlConnection conn = new SqlConnection(connectionString))
                {
                    childAdapter.SelectCommand = new SqlCommand("Select *from Produs where id_cat=@i");
                    childAdapter.SelectCommand.Parameters.Add("@i", SqlDbType.Int).Value = idcat_tb.Text;
                    childAdapter.Fill(childds, "Produs");
                    int x;
                    ...
                    childAdapter.UpdateCommand.Parameters.Add("@id", SqlDbType.Int).Value = childds.Tables["Produs"].Rows[childBS.Position][0];
                    conn.Open();
