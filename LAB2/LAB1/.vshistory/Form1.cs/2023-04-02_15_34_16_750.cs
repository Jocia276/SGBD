...
namespace LAB1
{
    public partial class Form1 : Form
    {
        ...
        private void update_prod_Click(object sender, EventArgs e)
        {
            try
            {
                using (SqlConnection conn = new SqlConnection(connectionString))
                {
                    ...
                    childAdapter.UpdateCommand.Parameters.Add("@id", SqlDbType.Int).Value = ds.Tables["Produs"].Rows[dataGridViewProdus.CurrentCell.RowIndex][0];
                    conn.Open();
        ...
        {
            childAdapter.SelectCommand = new SqlCommand("SELECT *FROM Produs where id_cat=@id");
            childAdapter.SelectCommand.Parameters.Add("@id", SqlDbType.Int).Value = ds.Tables["Categorie"].Rows[parentBS.Position][0];
            childds.Clear();
