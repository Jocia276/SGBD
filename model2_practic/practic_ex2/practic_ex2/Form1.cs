    ...
    public partial class Form1 : Form
    {
        ...
        private void buttonUpdate_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    int x;
                    childAdapter.UpdateCommand = new SqlCommand("Update Melodii set titlu=@nb, an_lansare=@d, durata=@p, cod_artist=@c where cod_melodie=@id", conn);
                    childAdapter.UpdateCommand.Parameters.Add("@nb", SqlDbType.VarChar).Value = textBox1.Text;
                    childAdapter.UpdateCommand.Parameters.Add("@d", SqlDbType.Int).Value = textBox2.Text;
                    childAdapter.UpdateCommand.Parameters.Add("@p", SqlDbType.Time).Value = textBox3.Text;
                    ...
                    if (x >= 1)
                    {
                        MessageBox.Show("Melodia a fost actualizata!");
                    }
                }
        ...
        private void buttonDelete_Click(object sender, EventArgs e)
        {
            try
            {
                int x;
                DialogResult dr;
                dr = MessageBox.Show("Are you sure you want to delete this song?", "Confirm Deletion", MessageBoxButtons.YesNo);
                if (dr == DialogResult.Yes)
                {
                    using (SqlConnection conn = new SqlConnection(connectionString))
                    {
                        childAdapter.DeleteCommand = new SqlCommand("Delete Melodii where cod_melodie=@id", conn);
                        ...
                        if (x >= 1)
                        {
                            MessageBox.Show("The song has been deleted");
                        }
                    }
                }
                else
        ...
        private void buttonAdd_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    int x;
                    childAdapter.InsertCommand = new SqlCommand("INSERT INTO Melodii VALUES (@nb, @d, @p, @id)", conn);
                    childAdapter.InsertCommand.Parameters.Add("@nb", SqlDbType.VarChar).Value = textBox1.Text;
                    childAdapter.InsertCommand.Parameters.Add("@d", SqlDbType.Int).Value = textBox2.Text;
                    childAdapter.InsertCommand.Parameters.Add("@p", SqlDbType.Time).Value = textBox3.Text;
                    ...
                    if (x >= 1)
                    {
                        MessageBox.Show("The song has been added!");
                    }
                }
            }
            catch (Exception ex)
